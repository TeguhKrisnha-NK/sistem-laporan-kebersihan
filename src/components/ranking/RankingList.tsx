...
export default function RankingList({ ranking }: { ranking: ClassRanking[] }) {
  ...
  // Helper Style untuk Juara 1, 2, 3
  const getRankStyle = (index: number) => {
    switch (index) {
      case 0: return 'bg-yellow-50 border-yellow-400 shadow-md ring-1 ring-yellow-200' // Emas
      case 1: return 'bg-gray-50 border-gray-300 shadow-sm' // Perak
      case 2: return 'bg-orange-50 border-orange-300 shadow-sm' // Perunggu
      ...
      {ranking.map((item, index) => (
        ...
        >
          ...
          <div className="flex-1 w-full">
            ...
            {/* Statistik Grid (Dibuat lebih lega) */}
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center sm:text-right border-l border-gray-200 pl-4">
                <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1">Performa</p>
                <p className={`text-xl font-bold ${item.persentase_bersih >= 80 ? 'text-green-600' : item.persentase_bersih >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
